...
/* ═══ أنشئ جداول المحتوى قبل أي ترقية أعمدة ═══
   proxy.js ينشئها أيضاً، لكن gateway.js يعمل أولاً — و IF NOT EXISTS يجعلها آمنة */
try {
...
function ensureColumn(table, col, ddl) {
  try {
    ...
    if (!t) return false;                     /* الجدول لم يُنشأ بعد */
...
/* أنشئ جدول الدروس إن لم يوجد ثم رقّه بأعمدة اللغتين */
try {
...
} catch (e) { console.warn('lessons table:', e.message); }
...
try {
  db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='questions'").get() &&
...
function setSetting(key, value) {
  ...
  } catch (e) { console.warn('setSetting skipped:', e.message); }
}
...
/* ═══ بذر الأسئلة الرسمية من PMI — يُستدعى بعد جهوز الجدول ═══ */
function seedOfficialQuestions() {
  try {
    const t = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='questions'").get();
    if (!t) return false;                       /* الجدول لم يُنشأ بعد */
    ...
    if (have >= rows.length) return true;       /* مبذورة سابقاً */
    const cols = db.prepare('PRAGMA table_info(questions)').all().map(x => x.name);
    /* حدّد باقات PMP أولاً — package_id قد يكون NOT NULL */
    let pks = [];
    try {
      pks = (getPackages() || []).filter(p => p.course === 'pmp' &&
        ['full','sim','review'].includes(p.type)).map(p => p.id);
    } catch (e) { pks = []; }
    if (!pks.length) pks = ['pmp-full', 'pmp-sim', 'pmp-review'];
    const want = ['id','course','type','domain','topic','difficulty','question_en','question_ar',
    ...
    /* أي عمود NOT NULL بلا قيمة افتراضية يجب ملؤه */
    const required = info.filter(c => c.notnull && c.dflt_value === null).map(c => c.name);
    const F = [...new Set(want.filter(c => cols.includes(c)).concat(
      required.filter(c => cols.includes(c))))];
    const ins = db.prepare(`INSERT OR REPLACE INTO questions (${F.join(',')})
      VALUES (${F.map(c => '@' + c).join(',')})`);
    let seq = 0;
    const tx = db.transaction(list => list.forEach(r => {
      const o = {
        package_id: pks[seq++ % pks.length],
        id: r.id, course: r.course, type: r.type, domain: r.domain,
        topic: r.topic, difficulty: r.difficulty,
        question_en: r.question_en || '', question_ar: r.question_ar || '',
        options_en: JSON.stringify(r.options_en || []),
        options_ar: JSON.stringify(r.options_ar || []),
        correct: r.correct || '', correct_json: JSON.stringify(r.correct_json || []),
        explanation_en: r.explanation_en || '', explanation_ar: r.explanation_ar || '',
        reference: r.reference || '', active: 1, is_official: 1, priority: 100,
        source_exam: r.source_exam || 'PMI Official', source_id: r.source_id || r.id
      };
      const p = {};
      F.forEach(c => {
        if (o[c] !== undefined) { p[c] = o[c]; return; }
        /* املأ الأعمدة الإلزامية التي لم نضع لها قيمة */
        if (c === 'options') p[c] = JSON.stringify(r.options_ar || r.options_en || []);
        else if (c === 'question') p[c] = r.question_ar || r.question_en || '';
        else if (c === 'explanation') p[c] = r.explanation_ar || r.explanation_en || '';
        else if (c === 'created' || c === 'updated') p[c] = Date.now();
        else p[c] = '';
      });
      ins.run(p);
    }));
    ...
    return true;
  } catch (e) { console.warn('PMI official seed:', e.message); return false; }
}
/* حاول فوراً، وإن لم يكن الجدول جاهزاً أعد المحاولة حتى ينشئه الخادم الداخلي */
if (!seedOfficialQuestions()) {
  let tries = 0;
  const t = setInterval(() => {
    if (seedOfficialQuestions() || ++tries > 40) clearInterval(t);
  }, 750);
}
...
function handleLearnerQuestionBank(req,res,url){
  ...
  /* الأسئلة الرسمية من PMI تتصدّر، ثم الباقي عشوائياً */
  const rows=db.prepare(`SELECT * FROM questions
