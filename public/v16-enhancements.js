...
function blogView(){
  ...
  if(slug){
    ...
    return '<article class="v16-article"><div class="wrap"><a class="v16-back" href="#blog" data-r="blog">العودة إلى المدونة</a><div class="v16-article-head">'+(p.image?'<img src="'+esc(p.image)+'" alt="'+esc(title)+'">':'')+'<span>'+esc(p.category||(lang==='en'?'Article':'مقال مهني'))+'</span><h1>'+esc(title)+'</h1><p>'+esc(p.author||'د. محمد عطية')+' · '+new Date(p.created).toLocaleDateString(lang==='en'?'en-US':'ar-SA')+'</p></div><div class="v16-article-body">'+String(body||'').split(/\n{2,}/).map(function(x){return '<p>'+esc(x).replace(/\n/g,'<br>')+'</p>'}).join('')+'</div></div></article>';
  }
  var english=window.__lang==='en';
  return '<section class="pg-hero v16-blog-hero"><div class="wrap"><div class="crumb"><a href="#home" data-r="home">' +(english?'Home':'الرئيسية')+'</a> ← '+(english?'Articles':'المدونة')+'</div><h1>'+(english?'Insights and articles':'المدونة المهنية')+'</h1><p>'+(english?'Practical ideas for projects, risk and professional development.':'مقالات تطبيقية من خبرة التدريب والاستشارات وإدارة المشاريع.')+'</p></div></section><section class="sec"><div class="wrap">'+(BLOGS.length?'<div class="v16-blog-grid">'+BLOGS.map(function(p){return '<article class="v16-blog-card">'+(p.image?'<img src="'+esc(p.image)+'" alt="'+esc(p.title_ar)+'">':'<div class="v16-blog-mark">AL</div>')+'<div><span>'+esc(p.category||'مقال')+'</span><h3>'+esc((window.__lang==='en'&&p.title_en)||p.title_ar)+'</h3><p>'+esc((window.__lang==='en'&&p.excerpt_en)||p.excerpt_ar||'')+'</p><a href="#blog/'+encodeURIComponent(p.slug)+'">'+(english?'Read article':'اقرأ المقال')+'</a></div></article>'}).join('')+'</div>':'<div class="note info">سيتم نشر المقالات قريباً.</div>')+'</div></section>';
}
