...
(() => {
  ...
  window.bindOverview=function(){priorBind();
    ...
    if(inbox)request('/api/admin/messages').then(rows=>{
      ...
      const states={new:'جديد',reviewing:'قيد المراجعة',replied:'تم الرد',closed:'مغلق'};
      inbox.innerHTML='<div class="badges" style="margin:15px 0"><span class="chip">إجمالي الرسائل · '+rows.length+'</span><span class="chip">جديد · '+rows.filter(r=>!r.status||r.status==='new').length+'</span><span class="chip">طلبات المدربين · '+trainer+'</span><span class="chip">الشراكات · '+partner+'</span></div>'+
        (rows.length?'<div class="tbl"><table><thead><tr><th>التاريخ</th><th>المرسل</th><th>الموضوع والرسالة</th><th>الحالة والرد</th></tr></thead><tbody>'+rows.map(r=>'<tr><td>'+esc(new Date(r.created).toLocaleDateString('ar-EG'))+'</td><td><b>'+esc(r.name)+'</b><br>'+esc(r.email)+'</td><td><b>'+esc(r.subject==='trainer_application'?'انضمام مدرب':r.subject==='strategic_partnership'?'شراكة استراتيجية':r.subject)+'</b><details><summary>اقرأ الرسالة</summary><p style="white-space:pre-wrap;max-width:55ch">'+esc(r.body||'')+'</p></details></td><td><label>الحالة<select data-message-status="'+esc(r.id)+'" aria-label="حالة رسالة '+esc(r.name)+'">'+Object.entries(states).map(([key,label])=>'<option value="'+key+'" '+((r.status||'new')===key?'selected':'')+'>'+label+'</option>').join('')+'</select></label><a href="mailto:'+encodeURIComponent(r.email)+'?subject='+encodeURIComponent('AL SAEED: '+r.subject)+'">رد بالبريد</a></td></tr>').join('')+'</tbody></table></div>':'<p class="muted">لا توجد رسائل محفوظة حتى الآن.</p>');
      inbox.querySelectorAll('[data-message-status]').forEach(select=>select.onchange=async()=>{
        const previous=rows.find(r=>r.id===select.dataset.messageStatus)?.status||'new';select.disabled=true;
        try{await request('/api/admin/messages/'+encodeURIComponent(select.dataset.messageStatus),{method:'PATCH',body:{status:select.value}});const row=rows.find(r=>r.id===select.dataset.messageStatus);if(row)row.status=select.value;toast('حُدثت حالة الرسالة')}
        catch(e){select.value=previous;toast(e.message||'تعذر تحديث الحالة')}finally{select.disabled=false}
      });
