...
(() => {
  ...
  document.addEventListener('input',event=>{if(event.target?.id!=='v36CatalogSearch')return;const query=event.target.value.trim().toLocaleLowerCase();const cards=[...document.querySelectorAll('.v36-catalog-grid .pk')];let visible=0;cards.forEach(card=>{const match=card.textContent.toLocaleLowerCase().includes(query);card.hidden=!match;if(match)visible++});const count=document.getElementById('v36CatalogCount');if(count)count.textContent=visible+' '+(document.documentElement.lang==='en'?'packages':'باقة');const empty=document.getElementById('v36CatalogEmpty');if(empty)empty.hidden=visible!==0});
