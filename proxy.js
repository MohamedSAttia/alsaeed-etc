...
function normalizePackage(body, old = {}) {
  ...
  /* الفصول: تقبل نصاً مفرداً (توافقاً) أو كائن {ar,en} */
  const normCh = arr => (Array.isArray(arr) ? arr : []).map(x => {
    if (x && typeof x === 'object') {
      const a = String(x.ar || '').trim(), e = String(x.en || '').trim();
      return (a || e) ? { ar: a, en: e } : null;
    }
    const t = String(x || '').trim();
    return t ? { ar: t, en: '' } : null;
  }).filter(Boolean);
  const chapters = body._chapters !== undefined ? normCh(body._chapters) : normCh(old._chapters || []);
  return {
    ...old,
    schemaVersion: 15,
    id,
